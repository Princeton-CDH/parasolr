...
To use ``parasolr`` you'll need to first create a solr core with the command
...
identifier is required::
   solr.update.index([{ "id": row["id"], # ... } for row in csv])
...
And then query the data::
   ...
   queryset = queryset.search(solr_query).order_by('some_var').continue_series_of_filters_and_maps()
