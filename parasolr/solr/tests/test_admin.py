...
class TestCoreAdmin:
    ...
    def test_create_unload(self, core_test_client):
        ...
        test_client.core_admin.unload(
            core, deleteInstanceDir=True, deleteIndex=True, deleteDataDir=True
        )
        ...
        with patch("parasolr.solr.admin.ClientBase.make_request") as mockrequest:
            test_client.core_admin.create(
                core, configSet=TEST_SOLR_CONNECTION["CONFIGSET"], dataDir="foo"
            )
    ...
    def test_ping(self, core_test_client, caplog):
        ...
        solrclient.core_admin.create(
            core, configSet=TEST_SOLR_CONNECTION["CONFIGSET"], dataDir="foo"
        )
        assert solrclient.core_admin.ping(core)

    def test_status(self, test_client):
        response = test_client.core_admin.status(
            core=TEST_SOLR_CONNECTION["COLLECTION"]
        )
